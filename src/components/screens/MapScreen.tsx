...
export default function MapScreen() {
  ...
  const [searchRadius, setSearchRadius] = useState(
    MAP_DEFAULTS.SEARCH.DEFAULT_RADIUS
  );
  ...
  const handleLocationSelect = (location: Location, address: string) => {
    ...
    if (mapRef.current) {
      ...
      mapRef.current.animateToRegion(
        region,
        GOOGLE_MAPS_CONFIG.ANIMATION_DURATION
      );
    }
  };
  ...
  const focusOnRunner = (runner: Runner) => {
    if (mapRef.current) {
      const region = createRegionFromLocation(runner.location, 0.5);
      mapRef.current.animateToRegion(
        region,
        GOOGLE_MAPS_CONFIG.ANIMATION_DURATION
      );
